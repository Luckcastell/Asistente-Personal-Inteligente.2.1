...
PREFIJO_DOCUMENTO = "DOCUMENTO_RAG: " # Importante para la indexación y recuperación
...
try:
    # Usamos un modelo local de HuggingFace para embeddings
...
# Inicializar la base de datos de vectores Chroma (persistencia habilitada)
vectorstore = Chroma(
...
def agregar_pdf_a_vectorstore(ruta_archivo: str, nombre_archivo: str):
    ...
    try:
        ...
        for doc in documentos:
            doc.metadata["source"] = nombre_archivo # Guardamos el nombre del archivo
            doc.page_content = PREFIJO_DOCUMENTO + doc.page_content # Añadimos el prefijo RAG
        ...
        # DEBUG: Imprimimos el conteo total para confirmar que se agregó algo
...
async def chat(solicitud: SolicitudChat):
    ...
    :param solicitud: Objeto SolicitudChat con el mensaje del usuario.
    :return: Un diccionario con la respuesta generada por Suriel.
    """
    try:
        # Paso 1: Recuperación (Retrieval) - Búsquedas Filtradas
        
        # Búsqueda 1: Recuperar Documentos RAG (excluye la memoria del chat)
        # Pedimos 9 trozos de documentos
        ...
        # Pedimos 6 trozos de memoria de chat
...
"""
        ...
        # Creamos un documento con el prefijo especial y el metadato 'Memoria del Chat'
